...
static int ape_tag_read_field(AVFormatContext *s)
{
    ...
    if (flags & APE_TAG_FLAG_IS_BINARY) {
        ...
        ret = avio_get_str(pb, size, (char *) filename, sizeof(filename));
        if (ret < 0)
            return ret;
        if (size <= ret) {
        ...
        av_dict_set(&st->metadata, (char *) key, (char *) filename, 0);

        if ((id = ff_guess_image2_codec((char *) filename)) != AV_CODEC_ID_NONE) {
            AVPacket pkt;
            int ret;
        ...
        value[c] = 0;
        av_dict_set(&s->metadata, (char *) key, (char *) value, AV_DICT_DONT_STRDUP_VAL);
    }
    return 0;
}
...
int64_t ff_ape_parse_tag(AVFormatContext *s)
{
    ...
    if (strncmp((char *) buf, (char *) APE_TAG_PREAMBLE, 8)) {
        return 0;
    }
...
int ff_ape_write_tag(AVFormatContext *s)
{
    ...
    while ((e = av_dict_get(s->metadata, "", e, AV_DICT_IGNORE_SUFFIX))) {
        ...
        if (!string_is_ascii((uint8_t *) e->key)) {
        ...
        avio_write(dyn_bc, (unsigned char *) e->value, val_len); // value
        count++;
    }
    if (!count)
        goto end;
    ...
    // header
    avio_write(s->pb, (unsigned char *) "APETAGEX", 8);   // id
